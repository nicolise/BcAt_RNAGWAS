...
Y = # nxp gene expression matrix, already normalized and ready to go!
  ...
  data = # data.frame with n rows giving the sample information, in the same order as Y
  ...
  K_geno = # n x n kinship matrix with rownames corresponding to genotype IDs
  # cis_genotypes = # list with p elements, each a n x b matrix with the local cis-genotypes for each of the p genes

  runID = # unique identifier for this chain. Should identify the model and the chain. Probably want to run at least 2 chains to compare.

  results_folder = #path to folder to store the results
