...
#pipeline note:
#1. run A01_prepGenome.R
#2. make sure there is a copy of plink executable in data/
#3. in command prompt: cd */data/
...
MyReads <- read.csv("data/Wei_Bc/result.lsm.csv") #9270 phenotypes
MyReads <- MyReads[,-c(1)]
...
Sys.time()
write.table(myFAM_match2, "data/coi1/02_GEMMA/binMAF20NA10.fam", row.names=FALSE, col.names=TRUE)
Sys.time()
