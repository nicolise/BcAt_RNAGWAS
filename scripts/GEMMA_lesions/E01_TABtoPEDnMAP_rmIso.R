...
#try removing isolate 01.01.12 prior to GEMMA: GEMMA drops SNPs??
rm(list=ls())
#on linux desktop
...
#remove PED row (01.01.12)
which(grepl("1.01.12", rownames(myPED))) #50
myPED2 <- myPED[-c(50),]

write.table(myMAP2, "data/B05_GEMMA_les/E_01_PLINK/dpbinMAF20NA10.map", row.names=FALSE, col.names=FALSE)
...
Sys.time()
write.table(myPED2, "data/B05_GEMMA_les/E_01_PLINK/dpbinMAF20NA10.ped", row.names=FALSE, col.names=FALSE)
write.csv(myPED, "data/B05_GEMMA_les/E_01_PLINK/dpbinMAF20NA10ped.csv")
Sys.time()
