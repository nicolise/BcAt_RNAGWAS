...
#also add a column for color coding in Wei's cluster 5
setwd("~/Projects/BcAt_RNAGWAS/")
WeiNet5 <- read.csv("data/fwas_botrytisnetworksno/net5.LSM.csv")
WeiNet5_list <- names(WeiNet5)[4:13]
#just gene name
WeiNet5_list <- substr(WeiNet5_list,1,nchar(WeiNet5_list)-2)

my.plotdat$Net5 <- ifelse(my.plotdat$transcript %in% WeiNet5_list, "Net5", "Other")

#remove any duplicate rows
my.plotdat <- unique(my.plotdat)
...
ggplot(my.plotdat, aes(x=SNP.Pos, y=Gene))+
  theme_bw()+
  geom_point(aes(colour=Net5), alpha=1/2)+
