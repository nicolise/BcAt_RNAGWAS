...
#Get genotype data
SNPs <- read.csv("allreadsGWAS/02_bigRR/lsmeans_testlowreads_SNPS_MAF20.csv", row.names = 1)
FullSNPs <- SNPs
SNPs <- FullSNPs
...
Phenos <- read.csv("allreadsGWAS/02_bigRR/lsmeans_testlowreads_phenos_MAF20.csv", row.names = 1)
...
con <- file("allreadsGWAS/03_bigRRout/testlowreads/lowreads_MAF20_013118.log")
sink(con, append=TRUE, type="message")

my.time1 <- print(Sys.time())
...
for(i in 1:dim(dat)[2]) { #i will be each isolate
  ...
  write.csv(outpt.HEM, file="allreadsGWAS/03_bigRRout/testlowreads/lowreads_MAF20_013118.csv", append=T)
}
